...
def drawer_write2(message2):
    (xx,yy) = cell_to_coord(10,height+1)
    yy += cell_size / 2
    drawer.goto(xx,yy)
    drawer.write(message2, font=("Arial", 12, "normal"), align="center")

def drawer_write3(message3):
    (xxx,yyy) = cell_to_coord(-10,height+1)
    yyy += cell_size / 2
    drawer.goto(xxx,yyy)
    drawer.write(message3, font=("Arial", 12, "normal"), align="center")

def drawer_write4(message4):
    (xxxx,yyyy) = cell_to_coord(0,height+5)
    yyyy += cell_size / 2
    drawer.goto(xxxx,yyyy)
    drawer.write(message4, font=("Arial", 12, "normal"), align="center")
...
def init_game():
    ...
    COLOR = [ "blue", "yellow", "pink", "orange", "purple" ]
    ...
    drawer_write2("LEFT = {} | SCORE = {}".format(len(items), score))
    drawer_write3("LIFE = {}" .format(life))
    drawer_write4("ENERGY={}".format(energy))
...
def update():
   ...
   if len(items) == 0:
      ...
      drawer.write("              VİCTORY!!\n LEFT LİFE = {} | SCORE = {}".format(life, score),align="center", font=("Comic Sans Ms", 25, "normal"))
        ...
        drawer.goto(0,0)
        drawer.color("red")
        drawer.write("Game Paused. Press 'p' to resume.", align="center",font=("Comic Sans Ms", 25, "normal"))
   else:
